...
\begin{itemize}
\item Scheduling is a fundamental concern in system design
\item CPUs and sequential execution
\item Definition of a function: synchronous + trivially schedulable
\item Definition of a process: concurrency + isolation
\item Cooperative multitasking and context switching
\item Timer interrupts and preemptive multitasking
\item Multiprocessing and parallelism
\item Definition of a thread: process-unlike resource sharing + function-unlike asynchrony
\item Asynchrony means scheduling managed by someone else, but user may not need parallelism
\item Threads are the standard application abstraction for exploiting preemption
\item Application programmers should have an easy abstraction for preemption
\end{itemize}
