...
finishing: \solb{Mention case studies built on top (incl. the safety ones) and other possibilities (incl. finishing them)}

applications: \solb{Other libgotcha uses:\@ aspect-oriented programming, multiple library versions}

applications: \solb{Other libinger use:\@ resource limits besides wall-clock time}

compatibility: \solb{Redesign global variable interception to leverage symbol sizes instead of disassembler and heuristics}

scalability: \solb{Port \textit{libgotcha} to drow}

scalability: \solb{Revisit startup time tradeoff if applications not long running and zygotes processes not appropriate}

performance: \solb{Make \textit{libtlsblock} robust and incorporate it into the system}

compatibility: \solb{Logic to avoid signal conflicts}

scalability: \solb{Remove signal scaling limit}

performance: \solb{Smarter time quanta and measurement study/profiler component}

defense in depth: \solb{Add option to insulate against preemptible function exceptions}

defense in depth: \solb{Systematic replacements to prevent circumventing isolation}

compatibility: \solb{Systematic replacements to hide library function semantics changes}

compatibility: \solb{Make \texttt{\_dl\_signal\_exception()} workaround robust and incorporate it into the system}

finishing: \solb{Complete implementation of \textit{ingerc} and integrate the runtime components into \textit{libinger}}

finishing: \solb{Port \textit{libturquoise} to modern Tokio, eliminating the compatibility glue code and properly supporting TLSes, as I've ported the main system to modern Rust and glibc}

defense in depth: \solb{Build a microservices platform on top of \textit{libinger}}
