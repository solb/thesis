...
outside the current object file to one of a number of possible copies.  It opens a
number of copies of the program's set of loaded libraries; we will refer to each of
these as a \textbf{libset}\footnote{Technically, a libset also includes the
executable, which is needed in case one of the application's libraries uses
preemptible functions that use symbols from the executable file.}.  Then, it uses
selective linking to weaken the semantics of either dynamic linking or preemption,
depending on the symbol being accessed.  In the common case, we weaken the former by
redirecting to a separate copy of the symbol in a ``personal'' libset, thereby
keeping any shared state that might become inconsistent separate from the rest of the
...
such cases, we instead direct the call to the main libset, and libinger defers
preemption until the call returns.
