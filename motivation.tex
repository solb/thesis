...
\solb{We were going to say that other IPC methods perform similarly; however, it
looks like sources suggest they can be a couple of microseconds faster.  I expect
the tail and throughput will still be bad; do you think we should (a) benchmark w/
Unix domain sockets [should be doable within an hour or so], (b) state this
hypothesis, or (c) not bring up other mechanisms?}
...
A 9\textmu{}s delay in microservice invocation is already a substantial fraction of
Azure's 20\textmu{}s RPC latency, a fraction that will only increase with
...
long (e.g., longer than 100\textmu{}s).  This preemption interval is orders of
