...
is by calling the \texttt{launch()} wrapper function means that there is only one way
for a preemptible function to run with its current libset set to the starting
...
\texttt{launch()} and \texttt{resume()}.  And as we saw in
Section~\ref{sec:gotcha:eval}, each global variable interception incurs several
microseconds of latency.  The same optimization should be equally applicable to other
