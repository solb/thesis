...
\solb{Move the accuracy study from the microservices chapter}

\solb{It must make sure to mention that you can always overrun by the quantum and warmup makes this worse}

\solb{It should move the chart from microservices to show what duration is practical}
...
\solb{Emphasize that the fixed quantum is not fundamental}
...
\begin{figure}
	...
	\end{lstlisting}
	\subcaption{\vspace{12pt} Example program}
	\end{minipage}
%
	\begin{minipage}{0.75\textwidth}
	\includegraphics[width=\textwidth]{figs/calltree_function_launch-crop}
	\subcaption{Corresponding inter-module control transfers (\texttt{callee()} is uninterruptible)}
	\end{minipage}
\caption{Cross-module function calls under \textit{libinger}. Color indicates uninterruptible code (i.e., next libset = starting libset).}
\end{figure}

\solb{Include discussion of deferred preemption}

\solb{Note that deferring without an active notification mechanism would be vulnerable to loosely tight loops that called such a function}
