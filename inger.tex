...
Before choosing the quantum to use, we were curious how small was possible under
...
Recall that some functions cannot be duplicated in this manner, and instead require
...
All benchmarks were run on an Intel Xeon E5-2683 v4 (Broadwell) server running
...
(Section~\ref{sec:libinger:tcbs}).  In both cases, \textit{libgotcha}'s global
variable interception was turned off.
...
to completion, they exhibit under two-thirds the latency of spawning a thread and are
an order of magnitude faster than forking a process.

\begin{table}
	\begin{minipage}{\textwidth}
	\centering
	\begin{tabular}{c | c c}
	Operation & Time with TLSes ($\mu{s}$) & Time without TLSes ($\mu{s}$) \\
	\hline
	\texttt{launch()} & $15.1 \pm 0.16$ & $12.4 \pm 0.15$ \\
	\texttt{resume()} & $13.9 \pm 0.07$ & $10.3 \pm 0.04$ \\
	\texttt{cancel()} & $31.7 \pm 0.48$ & $27.7 \pm 0.45$ \\
	\end{tabular}
	...
	Operation & Time ($\mu{s}$) \\
	\hline
	\texttt{fork()} & $492.0 \pm 7.32$ \\
	\texttt{pthread\_create()} & \;\;$24.7 \pm 7.45$
	\end{tabular}
...
\solb{Drop the reaper thread and rerun}

\solb{These old results are from Linux 4.12.6, rustc 1.36.0, and glibc 2.29}

\solb{Center the graph while at it}
