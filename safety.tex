...
\begin{figure}
\begin{lstlisting}[label=lst:acsafe,caption=\textit{libac-safe} source code]
struct reuse {
	bool available;
	libset_t libset;
};

static struct reuse reuse;
...
static void release(void *release) {
	...
	if(!reuse.available) {
		reuse.libset = wrapper->libset;
		__asm__ volatile("" ::: "memory");
		reuse.available = true;
	}
	free(wrapper);
}
...
int pthread_create(pthread_t *thread, const pthread_attr_t *attr, void *(*fun)(void *), void *arg) {
	...
	args->arg = arg;
	args->finished = false;
	if(atomic_compare_exchange_strong(&reuse.available, &acquired, false)) {
		assert(reuse.libset != LIBGOTCHA_LIBSET_STARTING);
		args->libset = reuse.libset;
		reuse.libset = LIBGOTCHA_LIBSET_STARTING;
	} else
